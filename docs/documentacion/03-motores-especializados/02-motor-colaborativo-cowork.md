...
* **Presencia de Docentes:** Controla la lista de docentes activos conectados a la redacción del PEA y el estado de su cursor en los campos colaborativos.
...
Previene colisiones directas de edición sobre un mismo bloque del PEA:
* Cuando un docente enfoca una sección específica (ej. *Resultados de Aprendizaje*, *Unidades Temáticas* o *Metodología*), emite una señal de bloqueo (`LockSection`).
...
CREATE TABLE doc_cowork_documentos (
    ...
    entidad_uuid VARCHAR(36) NOT NULL,    -- Identificador de la DocumentInstance / PEA
    campo_nombre VARCHAR(100) NOT NULL,   -- Nombre de la sección (ej. 'resultados_aprendizaje', 'contenidos')
