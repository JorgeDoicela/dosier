...
* **Just-In-Time (JIT) Provisioning:** 
  ...
  * **Superadministrador y Administrador Maestro (`Security:SuperAdminCedula` y `Security:MasterAdminId`):** Si la cédula o identificador del usuario coincide con las identidades declaradas en la configuración institucional (`appsettings.json` o variables de entorno), el sistema en `RbacService.SynchronizeUserRolesAsync` y en los scripts iniciales (`04_seguridad_rbac_roles_curriculares.sql`) asegura automáticamente el rol curricular máximo **`DOSIER_ADMIN`** de forma determinista e idempotente en `rbac_usuario_rol`, garantizando privilegios completos de administración independientemente del entorno (Staging local o AWS Producción).
