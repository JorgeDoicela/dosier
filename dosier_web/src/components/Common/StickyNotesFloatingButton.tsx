...
const getContextoLabel = (pathname: string): { label: string; Icon: React.ComponentType<{ size?: number }> } | null => {
    if (pathname.startsWith('/documentacion/proyectos')) return { label: 'Proyectos', Icon: Folder };
    if (pathname.startsWith('/documentacion/convocatorias')) return { label: 'Convocatorias', Icon: Megaphone };
    if (pathname.startsWith('/documentacion/monitoreo')) return { label: 'Monitoreo', Icon: BarChart3 };
    if (pathname.startsWith('/documentacion')) return { label: 'Documentación', Icon: FlaskConical };
