...
const DocumentEditor: React.FC<DocumentEditorProps> = ({ templateCode, initialData, entityUuid, onClose, readOnly = false, readOnlyReason, projectStatus, canSign = true }) => {
    ...
    useEffect(() => {
        const loadAll = async () => {
            ...
            const rawConfig = configResult?.data || configResult;
            ...
            if (genBlock && genBlock.config?.identificationMode === 'fields') {
                ...
                if (urlsToFetch.length > 0) {
                    const customFetches = await Promise.all(
                        urlsToFetch.map((url: string) => getCachedOrFetch(url, () => documentInstanceService.getCatalogByUrl(url)))
                    );
                    const catalogMap: Record<string, any[]> = {};
                    urlsToFetch.forEach((url: string, idx: number) => {
                        const fetched = customFetches[idx];
                        catalogMap[url] = Array.isArray(fetched) ? fetched : (fetched?.data || []);
                    });
            ...
            if (instanceData && (instanceData.uuid || instanceData.Uuid || instanceData.data_snapshot_json || instanceData.dataSnapshotJson)) {
                ...
                if (snapshotStr) {
                    try {
                        const parsed = typeof snapshotStr === 'string' ? JSON.parse(snapshotStr) : snapshotStr;
                        if (parsed) {
            ...
            // Aplicar catálogos
            const resolvedCarreras = Array.isArray(carrerasRes) ? carrerasRes : (carrerasRes?.data || []);
            setCarreras(resolvedCarreras);
...
const DocumentEditorCore: React.FC<DocumentEditorCoreProps> = ({
    ...
    const handleUpgradeTemplate = async () => {
        ...
        setIsUpgrading(true);
        try {
            await documentInstanceService.upgradeTemplate(documentId);
    ...
    const handleSave = async (data: any) => {
        try {
            ...
            if (cleanedData.Uuid) {
                const response = await documentInstanceService.updateMetadata(cleanedData.Uuid, cleanedData);
                ...
                const returnedUuid = response?.uuid || response?.data?.uuid;
