...
export function useProjectCore() {
    ...
    const templateCode = templateSlug ? slugToTemplateCode(templateSlug) : 'PEA_OFICIAL';
    ...
    const fetchProject = useCallback(async (onProjectFetched?: (data: any) => void) => {
        ...
        while (retries > 0 && !success) {
            try {
                const [projectDetailData, docsData] = await Promise.all([
                    curriculumProjectService.getProjectDetail(resolvedProjectUuid, isPeaTemplate),
                    documentInstanceService.getByEntity(resolvedProjectUuid)
                ]);
                res = { data: projectDetailData };
                resDocs = { data: docsData };
                success = true;
            } catch (e: any) {
                retries--;
    ...
    const resolveDocumentInstance = useCallback(async (docTemplateCode: string) => {
        ...
        try {
            ...
            const instanceData = await documentInstanceService.resolve({
            ...
            const instanceUuid = instanceData?.uuid || instanceData?.Uuid;
            if (instanceUuid) {
