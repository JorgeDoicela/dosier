...
export function useProjectTeam(
    ...
    const fetchTeamChangeRequests = useCallback(async (projectUuid?: string) => {
        ...
        try {
            const data = await curriculumProjectService.getTeamChangeRequests(uuidToUse);
            setTeamChangeRequests(data || []);
        } catch (err) {
        ...
        const fetchAvailableUsers = async () => {
            try {
                const [profRes, alumRes] = await Promise.all([
                    usersService.getUsers('type=DOCENTE&soloConHoras=false&pageSize=100'),
                    usersService.getUsers('type=ESTUDIANTE&origenEstudiante=TODOS&estadoEstudiante=TODOS&pageSize=100')
                ]);
                const mapUser = (u: any) => ({
    ...
    useEffect(() => {
        ...
        const delayDebounceFn = setTimeout(async () => {
            ...
            try {
                const query = `type=${targetType}&soloConHoras=false&estadoEstudiante=TODOS&origenEstudiante=TODOS&search=${encodeURIComponent(requestSearchQuery.trim())}&pageSize=30`;
                const res = await usersService.getUsers(query);
                const mapped = (res?.items || []).map((u: any) => ({
                    cedula: u.id_profesor || u.id_sigafi || '',
                    nombre: u.nombre_completo || u.nombre || '',
                    email: u.email || u.email_institucional || '',
                    carrera: u.carrera || '',
    ...
    useEffect(() => {
        ...
        const delayDebounceFn = setTimeout(async () => {
            ...
            try {
                const query = `type=DOCENTE&soloConHoras=false&search=${encodeURIComponent(transferSearchQuery.trim())}&pageSize=30`;
                const res = await usersService.getUsers(query);
                const mapped = (res?.items || []).map((u: any) => ({
                    cedula: u.id_profesor || u.id_sigafi || '',
                    nombre: u.nombre_completo || u.nombre || '',
    ...
    const handleConfirmTransfer = async (e: React.FormEvent) => {
        ...
        try {
            const res = await curriculumProjectService.transferDirector(currentProject.uuid, {
    ...
    const handleSaveTeam = async () => {
        ...
        try {
            ...
            const res = await curriculumProjectService.updateTeam(currentProject.uuid, payload);
            if (res.success) {
                addToast(
                    "Equipo de Trabajo",
    ...
    const handleCreateTeamChangeRequest = async () => {
        ...
        try {
            ...
            const res = await curriculumProjectService.createTeamChangeRequest(currentProject.uuid, payload);
            if (res?.success) {
    ...
    const handleReviewTeamChangeRequest = async (requestUuid: string, aprobar: boolean) => {
        if (!currentProject?.uuid) return;
        try {
            const res = await curriculumProjectService.reviewTeamChangeRequest(currentProject.uuid, requestUuid, {
                aprobar,
                ejecutar: aprobar,
                observacion_revision: aprobar ? "Aprobado por autoridad competente." : "Rechazado por autoridad competente."
            });
            if (res?.success) {
                addToast("Revisión completada", res.message || "Solicitud procesada.", "success");
                await fetchTeamChangeRequests(currentProject.uuid);
                const refreshed = await curriculumProjectService.getProjectDetail(currentProject.uuid);
                setInvestigadores((refreshed.investigadores || []).map(mapInvestigador));
            } else {
