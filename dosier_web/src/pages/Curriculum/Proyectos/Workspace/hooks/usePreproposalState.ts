...
export function usePreproposalState(
    ...
    const fetchTrazabilidad = useCallback(async (silent = false) => {
        ...
        try {
            const data = await curriculumProjectService.getTraceability(resolvedProjectUuid);
    ...
    useEffect(() => {
        ...
        const loadDocenteCarreras = async () => {
            try {
                const data = await curriculumProjectService.getDocenteCarreras();
    ...
    const handleGuardarYReenviar = async (nuevoTitulo: string, nuevaDescripcion: string, nuevoIdCarrera?: number) => {
        ...
        try {
            const docInstanceData = await documentInstanceService.resolve({
                templateCode: 'PROTOCOLO_INVESTIGACION',
                entityUuid: resolvedProjectUuid
            });
            const pInstanceUuid = docInstanceData?.uuid || docInstanceData?.Uuid;
            if (!pInstanceUuid) throw new Error("No se pudo resolver el expediente del protocolo.");

            const instanceData = await documentInstanceService.getById(pInstanceUuid);
            const currentMetadata = instanceData?.data_snapshot_json
                ? JSON.parse(instanceData.data_snapshot_json)
                : {};
            ...
            await documentInstanceService.updateMetadata(pInstanceUuid, updatedMetadata);

            await curriculumProjectService.transitionState(currentProject.uuid, 'Prepropuesta', 'Reenvío de prepropuesta corregida');
            ...
            addToast(
                ...
                async () => {
                    try {
                        await curriculumProjectService.transitionState(currentProject.uuid, 'Prepropuesta Rechazada', 'Reversión (Undo): Cancelación del reenvío de la prepropuesta.');
    ...
    const handleAdminAprobarPrepropuesta = async () => {
        ...
        try {
            ...
            await curriculumProjectService.transitionState(currentProject.uuid, 'Borrador', obs);
            addToast(
                "Idea Aprobada",
                ...
                async () => {
                    try {
                        await curriculumProjectService.transitionState(currentProject.uuid, 'Prepropuesta', 'Reversión (Undo): Cancelación de la aprobación de la prepropuesta.');
    ...
    const handleAdminDevolverPrepropuesta = async () => {
        ...
        setIsSubmittingAdminReview(true);
        try {
            await curriculumProjectService.transitionState(currentProject.uuid, 'Prepropuesta Rechazada', finalObservation);
            addToast(
                ...
                async () => {
                    try {
                        await curriculumProjectService.transitionState(currentProject.uuid, 'Prepropuesta', 'Reversión (Undo): Cancelación de la devolución de la prepropuesta.');
