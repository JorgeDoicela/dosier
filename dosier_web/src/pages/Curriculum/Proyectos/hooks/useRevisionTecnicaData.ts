    ...
    const loadTemplateBlocks = useCallback(async (uuid: string) => {
        try {
            ...
            if (loadedBlocks.length === 0) {
                try {
                    const uiConfigRes = await documentInstanceService.getTemplateUiConfig('PROTOCOLO_INVESTIGACION');
                    if (uiConfigRes?.sections && Array.isArray(uiConfigRes.sections)) {
                        setTemplateSections(uiConfigRes.sections);
                    }
                } catch {}

                const tmplRes = await documentTemplateService.getTemplateByCode('PROTOCOLO_INVESTIGACION');
