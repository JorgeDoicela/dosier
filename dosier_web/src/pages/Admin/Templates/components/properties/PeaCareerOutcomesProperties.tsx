...
import { Award, HelpCircle, Palette } from 'lucide-react';
import type { DocumentBlock } from '../../types';

interface PeaCareerOutcomesPropertiesProps {
    block: DocumentBlock;
    onUpdateConfig: (blockId: string, key: string, value: any) => void;
}

export const PeaCareerOutcomesProperties: React.FC<PeaCareerOutcomesPropertiesProps> = ({
    block,
    onUpdateConfig
}) => {
        ...
        <div className="space-y-4">
            ...
            <div className="bg-surface-subtle border border-border-thin rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-bold text-text-main uppercase tracking-wider">
                        Sección d) Resultados de Aprendizaje de la Carrera
                    </span>
                </div>
                <p className="text-[11px] text-text-dim leading-relaxed">
                    Articulación macro-curricular. Define los Resultados de Aprendizaje (RDA) del Perfil de Egreso de la Carrera a los que la asignatura aporta directamente.
                </p>
            </div>
            ...
            <div className="border border-border-thin rounded-lg p-3 space-y-3">
                ...
                <div className="space-y-1.5">
                    ...
                    <input
                        type="text"
                        value={c.rdaCarreraLabel ?? 'd)RESULTADOS DE APRENDIZAJE DE LA CARRERA A LOS QUE LA ASIGNATURA APORTA'}
                        onChange={(e) => handleConfigChange('rdaCarreraLabel', e.target.value)}
                ...
                <div className="space-y-1.5">
                    ...
                    <textarea
                        rows={3}
                        value={c.rdaCarreraPlaceholder ?? 'Resultados de aprendizaje del perfil de egreso a los que tributa la asignatura.'}
                        onChange={(e) => handleConfigChange('rdaCarreraPlaceholder', e.target.value)}
                        className="w-full bg-surface-subtle border border-border-thin rounded px-2.5 py-1.5 text-xs text-text-main focus:outline-none focus:border-violet-500"
                    />
                </div>
            ...
            <div className="flex items-start gap-2 p-2.5 bg-blue-500/5 border border-blue-500/20 rounded-lg text-[10.5px] text-blue-400">
                ...
                <span>
                    En el Workspace docente, este campo se enlaza a <code>&lt;CoWorkField&gt;</code> con clave <code>resultados_carrera</code>.
                </span>
            </div>
        </div>
    );
