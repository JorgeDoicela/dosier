...
import { CheckSquare, HelpCircle, Palette } from 'lucide-react';
import type { DocumentBlock } from '../../types';

interface PeaSubjectOutcomesPropertiesProps {
    block: DocumentBlock;
    onUpdateConfig: (blockId: string, key: string, value: any) => void;
}

export const PeaSubjectOutcomesProperties: React.FC<PeaSubjectOutcomesPropertiesProps> = ({
    block,
    onUpdateConfig
}) => {
        ...
        <div className="space-y-4">
            ...
            <div className="bg-surface-subtle border border-border-thin rounded-lg p-3">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs font-bold text-text-main uppercase tracking-wider">
                        Sección e) Resultados de Aprendizaje de la Asignatura
                    </span>
                </div>
            ...
            <div className="border border-border-thin rounded-lg p-3 space-y-3">
                ...
                <div className="space-y-1.5">
                    ...
                    <input
                        type="text"
                        value={c.rdaAsignaturaLabel ?? 'e) RESULTADOS DE APRENDIZAJE DE LA ASIGNATURA:'}
                        onChange={(e) => handleConfigChange('rdaAsignaturaLabel', e.target.value)}
                ...
                <div className="space-y-1.5">
                    ...
                    <textarea
                        rows={3}
                        value={c.rdaAsignaturaPlaceholder ?? 'Resultados de aprendizaje específicos alcanzables por el estudiante al finalizar el curso.'}
                        onChange={(e) => handleConfigChange('rdaAsignaturaPlaceholder', e.target.value)}
                        className="w-full bg-surface-subtle border border-border-thin rounded px-2.5 py-1.5 text-xs text-text-main focus:outline-none focus:border-violet-500"
                    />
                </div>
                ...
                <div className="space-y-1.5">
                    ...
                    <input
                        ...
                        max={10}
                        value={c.lineCount ?? 5}
                        onChange={(e) => handleConfigChange('lineCount', parseInt(e.target.value) || 5)}
                        className="w-full bg-surface-subtle border border-border-thin rounded px-2.5 py-1.5 text-xs text-text-main focus:outline-none focus:border-violet-500"
                    />
                </div>
            </div>
            ...
            <div className="flex items-start gap-2 p-2.5 bg-blue-500/5 border border-blue-500/20 rounded-lg text-[10.5px] text-blue-400">
                ...
                <span>
                    En el Workspace docente, este campo se enlaza a <code>&lt;CoWorkField&gt;</code> con clave <code>resultados_asignatura</code>.
                </span>
            </div>
        </div>
    );
