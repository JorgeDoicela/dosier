...
namespace dosier_api.Controllers
{
    ...
    public class ProjectsController : ControllerBase
    {
        ...
        public async Task<IActionResult> DevolverInformeFinal(
            ...
            [FromQuery] string observation,
            [FromQuery] string? fechaLimite,
            [FromServices] DosierContext context,
            [FromServices] dosier_application.Common.Notifications.INotificationService notificationService)
        {
            if (!await CanCurrentUserManageProjectAsync(id))
            {
            ...
            var project = await context.DocProyectos.FirstOrDefaultAsync(p => p.Uuid == id);
            ...
            var docInstances = await context.DocumentInstances
                .Where(d => d.EntityUuid == project.Uuid && d.TemplateCode == "INFORME_FINAL_INVESTIGACION")
                .ToListAsync();

            foreach (var doc in docInstances)
            {
                doc.ReopenForRevision();
                var firmas = await context.DocDocumentoFirmas
            ...
            try
            {
                var participantUserIds = await context.DocProyectoParticipantes
            ...
            {
                Console.WriteLine($"[DOSIER] Error al notificar devolución de informe final: {ex.Message}");
            }
